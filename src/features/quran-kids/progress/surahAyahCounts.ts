const AYAH_COUNTS_BY_SURAH_ID: readonly number[] = [
...
export const SURAH_AYAH_COUNT_BY_ID: Readonly<Record<number, number>> = Object.fromEntries(
  AYAH_COUNTS_BY_SURAH_ID.map((count, index) => [index + 1, count]),
);

export function getSurahAyahCount(surahId: number): number {
  return SURAH_AYAH_COUNT_BY_ID[surahId] ?? 0;
}
