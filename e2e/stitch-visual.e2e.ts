...
test.describe("Mounir Stitch shell visual contract", () => {
  ...
  test("desktop RTL shell places navigation right, content center, and discovery rail left", async ({ page, browserName }) => {
    ...
    await expect(actions.locator(".shell-control")).toHaveCount(0);
    ...
    await expect(accountMenu.locator("[role='menuitem']")).toHaveCount(4);
    ...
    const themeAction = accountMenu.locator("button[role='menuitem']").last();
    ...
    await accountMenu.locator("button[role='menuitem']").last().click();
